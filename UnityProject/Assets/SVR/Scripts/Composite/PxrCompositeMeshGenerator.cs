...
namespace Pxr.Composite
{
    ...
    [ExecuteAlways]
    public class PxrCompositeMeshGenerator : MonoBehaviour
    {
        private PxrComposite _Composite;
        ...
        private CompositeShape _LastShape;
        ...
        public void updateComposite(PxrComposite composite)
        {
        ...
        private void Update()
        {
            ...
            {
                var composite = GetComponent<PxrComposite>();
                if (composite == null)
                {
                    Debug.LogError("Need add PxrComposite");
                }
                else
                {
        ...
        {
            CompositeShape shape = _Composite.currentShape;
            Vector3 scale = _Transform.lossyScale;
            Texture texture = _Composite.texture;
            MeshFace meshFace = _Composite.meshFace;
            if (_Mesh == null ||
                _LastShape != shape ||
        ...
        private void UpdateMesh(CompositeShape shape, Vector3 scale, MeshFace meshFace)
        {
            if (_MeshFilter)
            {
                if(_Mesh == null)
                {
                    _Mesh = new Mesh() { name = "CompositeMesh" };
                }
                _Mesh.Clear();
                _Verts.Clear();
                _UV.Clear();
                ...
                ShapeGenerate.GenerateMesh(_Verts, _UV, _Tris, shape, meshFace, scale);
    ...
    class ShapeGenerate
    {
        public static void GenerateMesh(List<Vector3> verts, List<Vector2> uvs, List<int> tris, CompositeShape shape, MeshFace meshFace, Vector3 scale)
        {
            switch (shape)
            {
                case CompositeShape.Sphere:
                    BuildSphere(verts, uvs, tris, meshFace);
                    break;
                case CompositeShape.Quad:
                    BuildQuad(verts, uvs, tris, meshFace);
                    break;
                case CompositeShape.Cylinder:
