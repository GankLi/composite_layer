...
class SvrPluginAndroid : SvrPlugin
{
    ...
    [DllImport("svrplugin")]
    private static extern bool SvrPollEvent(ref int eventType, ref uint deviceId, ref float eventTimeStamp, int eventDataCount, uint[] eventData, ref float halfIpd);
    ...
    [DllImport("svrplugin")]
    private static extern void SvrGetPredictedControllerPose(int handle, bool isMultiThreadedRender, ref int poseStatus, ref float rotationX, ref float rotationY, ref float rotationZ, ref float rotationW, ref float positionX, ref float positionY, ref float positionZ);
    ...
    [DllImport("svrplugin")]
    private static extern bool SvrAnchorSave(AnchorUuid id);
    ...
    [DllImport("svrplugin")]
    private static extern bool SvrAnchorRelocatingStart();

    [DllImport("svrplugin")]
    private static extern bool SvrAnchorRelocatingStop();

    [DllImport("svrplugin")]
    private static extern bool SvrAnchorRemoveAllAnchorsFile();
    //---------------------------------------------------------------------------------------------
    ...
    //---------------------------------------------------------------------------------------------
    [DllImport("svrplugin")]
    private static extern void PvrRequestBoundaryVisible(bool visible);

    [DllImport("svrplugin")]
    private static extern void PvrSetBoundaryDisable(bool disable);
    ...
    public override IEnumerator EndVr()
	{
        yield return base.EndVr();
    ...
    public override bool PollEvent(ref SvrManager.SvrEvent frameEvent)
    {
		...
		int eventType = 0;
        float halfIpd = 0;
        bool isEvent = SvrPollEvent(ref eventType, ref frameEvent.deviceId, ref frameEvent.eventTimeStamp, dataCount, dataBuffer, ref halfIpd);
        ...
        switch (frameEvent.eventType)
        {
            ...
            case SvrManager.svrEventType.kEventProximity:
                //Debug.LogFormat("PollEvent kEventProximity : data {0} {1}", dataBuffer[0], dataBuffer[1]);
            ...
            case SvrManager.svrEventType.kEventIPD:
                //Debug.LogFormat("PollEvent: data {0}", halfIpd);
                frameEvent.eventData.ipd.halfIpd = halfIpd;
                break;
        }
        return isEvent;
    }
    ...
    /// <summary>
    /// Controllers get the predicted pose
    /// </summary>
    /// <param name="handle">Handle.</param>
    /// <param name="state">state.</param>
    //---------------------------------------------------------------------------------------------
    public override void ControllerGetPredictedPose(int handle, ref SvrControllerPredictedPose state)
    {
        SvrGetPredictedControllerPose(handle, SystemInfo.graphicsMultiThreaded, ref state.poseStatus, ref state.rotation.x, ref state.rotation.y, ref state.rotation.z, ref state.rotation.w, ref state.position.x, ref state.position.y, ref state.position.z);
        state.rotation.x = -state.rotation.x;
        state.rotation.y = -state.rotation.y;
        state.position.z = -state.position.z;
    }
    ...
    {
        return SvrAnchorRelocatingStart();
    }
    ...
    public override bool AnchorRemoveAllFiles()
    {
        return SvrAnchorRemoveAllAnchorsFile();
    }

    public override void RequestBoundaryVisible(bool visible)
    {
        PvrRequestBoundaryVisible(visible);
    }

    public override void SetBoundaryDisable(bool disable)
    {
        PvrSetBoundaryDisable(disable);
    }

    public override void GetBoundaryPlayArea(ref float minX, ref float maxX, ref float minY, ref float maxY, ref float minZ, ref float maxZ, ref float visibilityRadius)
    {
        PvrGetBoundaryPlayArea(ref minX, ref maxX, ref minY, ref maxY, ref minZ, ref maxZ, ref visibilityRadius);
