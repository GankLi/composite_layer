...
public class SvrOverlay : MonoBehaviour, IComparable<SvrOverlay>
{
    ...
    private int frustumType = (int)SvrManager.SvrSettings.eFrustumType.Camera;
    ...
    void UpdateCoords()
    {
        ...
        var worldLowerLeft = new Vector4(-layerWidth/2, -layerHeight/2, 0, 1);
        var worldUpperLeft = new Vector4(-layerWidth / 2, layerHeight / 2, 0, 1);
        var worldUpperRight = new Vector4(layerWidth / 2, layerHeight / 2, 0, 1);
        var worldLowerRight = new Vector4(layerWidth / 2, -layerHeight / 2, 0, 1);
        Matrix4x4 MVP = viewCamera.projectionMatrix * viewCamera.worldToCameraMatrix * imageTransform.localToWorldMatrix;
        //Debug.Log("-------------Begin UpdateCoords-------------");
        //Debug.Log("Model      Matrix: " + imageTransform.localToWorldMatrix.ToString());
        //Debug.Log("View       Matrix: " + viewCamera.worldToCameraMatrix.ToString());
