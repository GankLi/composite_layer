...
public class SvrManager : MonoBehaviour, SvrManager.SvrEventListener
{
    ...
    public enum svrEventType
	{
        ...
        kEventProximity = 19,
        kEventVrModeHeadless = 20,
        kEventIPD = 21,
        kEventOutsideBoundarySystem = 22,
        kEventInsideBoundarySystem = 23
    };
    ...
    { 
        ...
        [Tooltip("turn off the guardian system")]
        public bool disableBoundary = false;
    ...
    public struct svrEventData_IPD
    {
        public float halfIpd;
    };
    ...
    private int vrState = 1; //TODO： 暂时先这样，0表示暂停，1表示运行中
	...
	IEnumerator Initialize()
	{
        ...
        plugin.SetBoundaryDisable(settings.disableBoundary);
    }
    ...
    private void RefreshIPD(float halfIpd)
    {
        SvrPlugin.DeviceInfo temp = plugin.GetDeviceInfo();
        plugin.deviceInfo.targetFrustumLeft.position.x = -halfIpd;
        plugin.deviceInfo.targetFrustumRight.position.x = halfIpd;

        plugin.deviceInfo.targetFovXRad = temp.targetFovXRad;
        plugin.deviceInfo.targetFovYRad = temp.targetFovYRad;

        plugin.deviceInfo.targetFrustumLeft.left = temp.targetFrustumLeft.left;
        plugin.deviceInfo.targetFrustumLeft.right = temp.targetFrustumLeft.right;
        plugin.deviceInfo.targetFrustumLeft.top = temp.targetFrustumLeft.top;
        plugin.deviceInfo.targetFrustumLeft.bottom = temp.targetFrustumLeft.bottom;

        plugin.deviceInfo.targetFrustumRight.left = temp.targetFrustumRight.left;
        plugin.deviceInfo.targetFrustumRight.right = temp.targetFrustumRight.right;
        plugin.deviceInfo.targetFrustumRight.top = temp.targetFrustumRight.top;
        plugin.deviceInfo.targetFrustumRight.bottom = temp.targetFrustumRight.bottom;

        foreach (SvrEye eye in eyes)
        {
            eye.InitializeCameras();
        }
    ...
    private SvrPlugin.Foveation GetFoveationValues()
    {
        ...
        switch (settings.foveationLevel)
        {
            ...
            case SvrSettings.eFoveationLevel.None:
                ...
                foveationValues.Gain = Vector2.zero;
    ...
    IEnumerator OnPause()
	{
        ...
        /*if (plugin.IsRunning())*/
        yield return StartCoroutine(plugin.EndVr());
        onResume = null;
    }

    IEnumerator OnResume()
	{
        Debug.Log("SvrManager.Resume()");
    ...
    void LateUpdate()
    {
        ...
        if (!disableInput)
        {
            ...
            {
                head.transform.localPosition = headRecenterRotation * (headRecenterPosition + headPose.position * settings.trackPositionScale);
            }
    ...
    public void OnSvrEvent(SvrManager.SvrEvent ev)
    {
        switch (ev.eventType)
        {
            case SvrManager.svrEventType.kEventProximity:
                Debug.LogFormat("OnSvrEvent: Proximity Distance {0}, State {1}", ev.eventData.proximity.distance, ev.eventData.proximity.distance > 0.0f);
            ...
            case SvrManager.svrEventType.kEventVrModeStarted:
                break;
            case SvrManager.svrEventType.kEventIPD:
                Debug.LogFormat("OnSvrEvent: IPD change {0}", ev.eventData.ipd.halfIpd);
                RefreshIPD(ev.eventData.ipd.halfIpd);
                break;
            case SvrManager.svrEventType.kEventBoundarySystemCollision:
                float minX = 0.0f;
                float maxX = 0.0f;
                float minY = 0.0f;
                float maxY = 0.0f;
                ...
                plugin.GetBoundaryPlayArea(ref minX, ref maxX, ref minY, ref maxY, ref minZ, ref maxZ, ref visibilityRadius);
                Debug.LogFormat("OnSvrEvent: Boundary has inited! minX = {0}, maxX = {1}, minY = {2}, maxY = {3}, minZ = {4}, maxZ = {5}, visibilityRadius = {6}", minX, maxX, minY, maxY, minZ, maxZ, visibilityRadius);
                break;
        }
    }
    ...
    {
        Debug.LogFormat("SubmitEvent: {0}", frameEvent.eventType.ToString());
        for (int i = 0; i < eventListeners.Count; i++)
        {
            eventListeners[i].OnSvrEvent(frameEvent);
        }
    ...
    public void ControllerGetPredictedPose(int handle, ref SvrControllerPredictedPose state) 
    {
        plugin.ControllerGetPredictedPose(handle, ref state);
    }      
